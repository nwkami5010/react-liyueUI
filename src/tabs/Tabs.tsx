...
const RootTab:FC<TabsProps> =({
  ...
  const navClass = classNames('l-tabs-nav',{
    'nav-line': type === 'line',
    'nav-card': type === 'card'
  })
  ...
  const renderNavLinks = () =>
    React.Children.map(children,(child,index)=> {
      const childElement = child as
        FunctionComponentElement<TabsProps>
      const {tab,disabled} = childElement.props
      const classes = classNames('l-tabs-nav-item',{
  ...
  return (
    <div className={`l-tabs ${className}`} >
      <ul className={navClass}>
        {renderNavLinks()}
      </ul>
      <div className="l-tabs-content">
        {renderContent()}
      </div>
